...
//indexOf() - Даёт позицию начала заданной строки в той строке, на которой применяется метод. Возвращает -1, 
...
//lastIndexOf() - Даёт позицию последнего появления строки str в актуальной строке. Возвращает -1, 
//если поисковая строка не найдена.
...
// slice() - Отдает новую строку, которая является частью строки на которой применялся метод, от позиций begin до end. 
...
//trim(), trimEnd(), trimStart() - возвращает новую строку удаляя пробелы вначале и в конце, 
//или только в начале, ии только в конце оригинальной строки.
...
//some() - Этот метод проверяет есть ли нужные элементы в массиве и возвращает true и false. 
//Это очень похоже по концепции на метод includes(), за исключением того, что аргументом является функция, а не строка.
...
// map() - Этот метод схож с filter() в плане выдачи нового массива. Однако, разница тут в том, 
//что он используется только для изменения элементов.
